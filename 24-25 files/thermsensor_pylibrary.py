...
for device in dev_folder_list:
    ...
    sensors.append(W1ThermSensor(sensor_id = curr_id[3:]))
...
while True:
    ...
    # #start = time.time()
    # temp_c, temp_f = read_temp(sensors[0])
    # print(f"Temperature for S1: {temp_c:.2f}°C / {temp_f:.2f}°F")
    # #end1 = time.time()
    
    # temp_c, temp_f = read_temp(sensors[1])
    # print(f"Temperature for S2: {temp_c:.2f}°C / {temp_f:.2f}°F")

    # temp_c, temp_f = read_temp(sensors[2])
    # print(f"Temperature for S3: {temp_c:.2f}°C / {temp_f:.2f}°F")
    # #end2 = time.time()

    for i in range(num_files):
        temp_c, temp_f = read_temp(sensors[i])
        print(f"Temperature for S{i+1}: {temp_c:.2f}°C / {temp_f:.2f}°F")
